...
export function SumPayment(props) {
  ...
  const payList = [
    {name: 'b0', title: '==Các bậc giá điện========', value: '========THÀNH TIỀN (vnd)=='},
  ...
  React.useEffect(() => {
    if (isTotal) {
      ...
      let i = 1;
      do {
        console.log('sodien',sodien, values)
        if (sodien > 0) {
          if (i === 1) {
            console.log('thuc hien 1',sodien > 50 * house ? 50 * house * 1678 : sodien * 1678)
      ...
      setValues({b1,b2,b3,b4,b5,b6,notVat,vat,total});
    } else{
      setValues(null);
    }
  }, [isTotal]);
  ...
  return (
    <Grid>
      {payList.map((elm) => (
        <Box key={elm.name} sx={{display: 'flex', justifyContent: 'space-between'}}>
